...
def init_db(conn):
    ...
    cursor.execute("CREATE TABLE query_embeddings (rowid INTEGER PRIMARY KEY, embedding BLOB)")

    # Initialize for quantization
    try:
        cursor.execute("SELECT vector_init('query_embeddings', 'embedding', 'type=INT8,dimension=768');")
    ...
    try:
        ...
        for query_name, query_data in queries.items():
            if isinstance(query_data, str):
                 ...
                 platform = query_data.get("platform", "all")
            ...
            if sql:
                ...
                cursor.execute("SELECT rowid FROM query_library WHERE name = ? AND pack = ?", (query_name, pack_name))
                row_result = cursor.fetchone()
                if row_result:
                    rowid = row_result[0]
                    embed_text = f"Name: {query_name}\nDescription: {desc}\nRationale: {val}\nSQL: {sql}"
                    cursor.execute("INSERT OR REPLACE INTO query_embeddings(rowid, embedding) VALUES (?, llm_embed_generate(?))", (rowid, embed_text))
                    count += 1
...
def populate_fts(conn):
    ...
    conn.execute("INSERT INTO query_library_fts (name, pack, query, description, value, platform) SELECT name, pack, query, description, value, platform FROM query_library")
    conn.commit()
