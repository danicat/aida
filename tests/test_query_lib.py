...
class TestQueryLibrary(unittest.TestCase):
    def test_memory_info_no_platform(self):
        ...
        results = search_query_library("memory info")
        print(results)
    ...
    def test_memory_info_darwin(self):
        ...
        results = search_query_library("memory info", platform="darwin")
        print(results)
        self.assertIsInstance(results, list)
        self.assertTrue(len(results) > 0)
    ...
    def test_memory_info_linux(self):
        ...
        results = search_query_library("memory info", platform="linux")
        print(results)
        self.assertIsInstance(results, list)
        self.assertTrue(len(results) > 0)
        self.assertTrue("memory" in str(results).lower())
