...
*   **Context Management:**
    *   **Indicator:** Add a visual bar or percentage showing current context window usage.
    *   **Controls:** Add ability to `/clear` current context or "compress" it (summarize history to free up space).

## 2. Functional Enhancements
*   **Runtime Model Switching:** Expose the hidden `/model` slash command (currently supports `gemini` and `qwen`) via a UI toggle in the sidebar.
...
*   **"Active Tool" Indicator:** Visual feedback in the UI when a long-running tool (like a complex `osquery` scan) is executing. (Partially implemented with text status, could be enhanced with icons).
